...
void do_bgfg(char **argv) 
{
    ...
    char* id_str = argv[1];//获取第二个参数
    struct job_t* job;
    if(id_str[0] == '%')//JID
    {
        //char* jid_str = id_str;
        int jid = atoi(id_str+1);//字符串转int
    ...
    else if(strspn(id_str,"0123456789")==strlen(id_str))//PID （看第二个参数是否全是数字）
    {
        int pid = atoi(id_str);
